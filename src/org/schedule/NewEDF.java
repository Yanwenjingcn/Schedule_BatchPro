...
public class NewEDF {
	...
	public static ArrayList<DAG> dag_queue; // DAG队列
	public static ArrayList<DAG> dag_queue_ori; // 初始DAG队列
	...
	public static DAGdepend dagdepend; // DAG图之间的依赖关系

	public static int[][] petimelist; // 每次任务运行的结束时间
	public static int[] petimes; // 每个处理器上运行完成的任务个数

	public static int pe_number; // 处理器数量
	public static int proceesorEndTime = BuildParameters.timeWindow;// 时间窗
	public static int timeWindow; // 每个处理器存活的时间

	public NewEDF(int PEnumber) {
	 ...
	 */
	private static boolean checkready(DAG dag, ArrayList<DAG> queue1,
		...
		if (dag.getpass() == false && dag.getdone() == false) {
			if (current >= dag.getdeadline()) {
				dag.setpass(true);//设置该DAG任务已经过期，忽略
			}
			if (dag.getstart() == 0 && dag.getpass() == false) {
				...
				if (pre.size() >= 0) {
					for (int j = 0; j < pre.size(); j++) {
						...
						// 判断当前DAG的父DAG是否被取消，若是，则当前DAG也会被取消
						if (buf3.getpass()) {
							isready = false;
							dag.setpass(true);
							break;
	...
	public static int[] makespan(int PEnumber) throws Throwable {
		...
		for (DAG dag_ : dag_queue_ori) {
			...
			dag_new.setnewdeadline(dag_.getnewdeadline());
	 ...
	 *            ，当前时刻
	 */
	private static void schedule(ArrayList<DAG> queue1, DAGdepend dagdepend,
			int current) {
	...
	private static void choosePE(DAG dag_temp) {
		...
		for (int i = 0; i < pe_list.size(); i++) {
			if (pre_queue.size() == 0) {//如果该DAG没有父DAG，应该就是这个DAG是第一个提交DAG
				//如果当前时间>当前处理器上一次任务执行结束时间（petimelist[i][petimes[i]]），
				//	设置该处理器能处理当前DAG的最早时间为current_time
				//	否则，该处理器能处理当前DAG的最早时间为该处理器执行完上一个任务的时间
				if (current_time > petimelist[i][petimes[i]])
					temp[i] = current_time;
				else
					temp[i] = petimelist[i][petimes[i]];
			} else if (pre_queue.size() == 1) {//如果当前DAG只有一个父DAG
				if (pre_queue.get(0).getpeid() == pe_list.get(i).getID()) {//当前处理器为父节点所在的处理器。不考虑数据传输时间
					if (current_time > petimelist[i][petimes[i]])
						temp[i] = current_time;
					else
						temp[i] = petimelist[i][petimes[i]];
				} else {//当前处理器不为父节点所在的处理器。考虑数据传输时间
			...
			} else {//如果当前DAG有多个父DAG，找寻能最晚开始DAG执行的处理器？？？？？？？
				int max = current_time;
				for (int j = 0; j < pre_queue.size(); j++) {
					if (pre_queue.get(j).getpeid() == pe_list.get(i).getID()) {//当前处理器为父节点所在的处理器。不考虑数据传输时间
						if (max < petimelist[i][petimes[i]])
							max = petimelist[i][petimes[i]];
					} else {//当前处理器为父节点所在的处理器，考虑数据传输时间
		...
		if (min < dag_temp.getdeadline()) {
			...
			dag_temp.setpass(true);//设置这个DAG过期失效，整个取消
		}
	 ...
	 * 
	 * @Title: sort
	 * @Description: 按照到达时间从小到达的顺序排序
	 * @param @param ready_queue
	 * @param @param course_num
	 * @param @throws Throwable
	 * @return void
	 * @throws
	 */
	private static void sort(ArrayList<DAG> ready_queue, int course_num)
			throws Throwable {
		ArrayList<DAG> buff = new ArrayList<DAG>();
		DAG min = new DAG();
		DAG temp = new DAG();
